class FaultyTerminalApp {
  static init() {
    ...
    // We remove the inline styles here so the container dictates the layering
    ...
    // Use orthographic camera for a 2D full-screen shader
    ...
    const fragmentShader = `
      ...
      float digit(vec2 p){
          ...
          if(uUseMouse > 0.5){
              ...
              // Increased multiplier from 8.0 to 20.0 to make the cursor glow radius much smaller
      ...
      vec3 getColor(vec2 p){
          // Removed the sweeping refresh bar, keeping a flat intensity for the glow
          float bar = 1.0;
    ...
    this.uniforms = {
      ...
      uGlitchAmount: { value: 0 }, // Changed from 1 to 0 to completely disable the sweeping displacement glitch
  ...
  static animate() {
    ...
    const elapsed = this.clock.getElapsedTime() * 1.0; // timeScale = 1
